...
describe('agentRequest.store', () => {
	...
	describe('Getters', () => {
		it('gets parameter overrides for a node', () => {
			...
			store.setAgentRequestForNode('workflow1', NODE_ID_1, {
				query: { param1: 'value1', param2: 'value2' },
			});

			const overrides = store.getAgentRequests('workflow1', NODE_ID_1);
		...
		it('gets a specific parameter override', () => {
			...
			const override = store.getQueryValue('workflow1', NODE_ID_1, 'param1');
		...
		it('returns undefined for non-existent parameter', () => {
			...
			const override = store.getQueryValue('workflow1', NODE_ID_1, 'non-existent');
		...
		it('handles string query type', () => {
			...
			const query = store.getAgentRequests('workflow1', NODE_ID_1);
	...
	describe('Actions', () => {
		it('sets parameter overrides for a node', () => {
			...
			expect(
				(store.agentRequests.value['workflow1'] as unknown as { [key: string]: IAgentRequest })[
					NODE_ID_1
				].query,
			).toEqual({
				param1: 'value1',
		...
		it('clears parameter overrides for a node', () => {
			...
			store.setAgentRequestForNode('workflow1', NODE_ID_2, {
				query: { param3: 'value3' },
			});

			store.clearAgentRequests('workflow1', NODE_ID_1);
		...
		it('clears all parameter overrides for a workflow', () => {
			...
			store.clearAllAgentRequests('workflow1');

			expect(store.agentRequests.value['workflow1']).toEqual({});
