				...
				if (resource === 'attendee') {
					...
					if (operation === 'create') {
						...
						if (additionalFields.multiChoiceResponses) {
							const multiChoiceResponses = additionalFields.multiChoiceResponses as any;
							const responses = body.responses as any[];
							if (Array.isArray(responses) && multiChoiceResponses?.details) {
								responses.push.apply(responses, multiChoiceResponses.details);
							}
